...
def index():
	...
	questions_cur = db.execute('''select 
										questions.id as question_id, questions.question_test, askers.name as asker_name, experts.name as expert_name 
								from questions 
...
def question(question_id):
	...
	question_cur = db.execute('''select 
										questions.question_test, questions.answer_text,askers.name as asker_name, experts.name as expert_name 
								from questions 
								join users as askers on askers.id = questions.asked_by_id 
								join users as experts on experts.id = questions.expert_id 
...
def answer(question_id):
	...
	question_cur = db.execute('select id, question_test from questions where id = ?', [question_id])
	question = question_cur.fetchone()
...
def ask():
	...
	if request.method == 'POST':
		db.execute('insert into questions (question_test, asked_by_id, expert_id) values (?, ?, ?)', [request.form["question"], user['id'], request.form["expert"]])
		db.commit()
...
def unanswered():
	...
	question_cur = db.execute('''select 
										questions.id, questions.question_test, users.name 
								from questions 
